...
required.packages <- c('rJava','Rdrools','DT','lubridate','tidyverse','plotly')
...
**Plotting graphs of result obtained**
  ...
  if(plotName == "Plot of points distribution"){
    ...
    map (1:length(result),function(i){
      ...
      if(nrow(noOfTrueFalse)==2){
        ...
        anomaliesCountPlot[[i]] <- plot_ly(noOfTrueFalse, x=~IsTrue, y=~Frequency,
                                           type = "bar", width=800, height=500,marker = list(color = 'rgba(158,202,225, 1)',
                                                                                             line = list(color = 'rgba(8,48,107, 0)'))) %>%
          layout(title = paste0("Distribution of points for rule "),
                 xaxis = list(title = ""),
                 yaxis = list(title = "Count"), margin = list(b = 80,t=50))
      }else{
    ...
    plotAnomalies <-list()
    map (1:length(result),function(ruleNum){
      ...
      if(class(intermediateOutput)=="list"){
        ...
        intermediateOutput<- filter_(intermediateOutput,paste(ruleName,"==","'true'"))
        
        GroupedCols <- paste(colnames(intermediateOutput[ , !names(intermediateOutput) %in% c(ruleName,ruleValue) ]),collapse = ":")
        ...
        plotAnomalies[[ruleNum]] <- plot_ly(intermediateOutput, x=~Group, y=~values, type="bar", width=800, height=500,
                                            marker = list(color = 'rgba(158,202,225, 1)',
                                                          line = list(color = 'rgba(8,48,107, 0)',
                                                                      width = 2))) %>%
          layout(title = paste0("Groups satisfying the rule "),width = 900,
                 xaxis = list(title = paste0("Grouped By - ",GroupedCols), tickangle = -45),
                 yaxis = list(title = "Aggregated Value"), margin = list(b = 180,t=50))
...
Here we compare values of two columns. In the case of the **iris** dataset, we compare the *Petal.Length* with *Sepal.Width*, and identify the *rows* which have an *Petal.Length* greater than *Sepal.Width* . 
...
* **intermediateOutput** : intermediate output is an empty list as there is no aggregation
* **output**: has the output dataframe with 3 columns:
    * **Group**: the above rule has no group by and filter. Hence the rule is applied on each row. Group in this case represents the row
    * **Indices**: the row numbers of the each group
    * **IsTrue**: flag to say if the datapoint is satisfying the rule or not. In this case, Flag is *true* if the *Petal.Length* greater than *Sepal.Width*   and *false* if it's not
...
Here we compare values of two columns after filtering the dataset. In the case of the **iris** dataset, we compare the *Petal.Length* with *Sepal.Width*, and identify the *rows* which have an *Petal.Length* greater than *Sepal.Width* . 
...
* **intermediateOutput** : intermediate output is an empty list as there is no aggregation
* **output**: has the output dataframe with 3 columns:
    * **Group**: the above rule has no group by. Hence the rule is applied on each row. Group in this case represents the row
    * **Indices**: the row numbers of the each group
    * **IsTrue**: flag to say if the datapoint is satisfying the rule or not. In this case, Flag is *true* if the *Petal.Length* of *Versicolor* species is greater than *Sepal.Width*   and *false* if it's not
...
```{r,warning=FALSE}
transactionData <- read.csv("../data/SampleDataset/TransactionsData.csv",stringsAsFactors = F)
...
```{r,comment=NA}
transactionRules <- read.csv("../data/SampleRules/workingRules.csv")
...
The distribution of points i.e, the *Account_ID* that are true or false is shown by the graph below. In this case, the true values can be called as **Anomalous Account_IDs** and the points that are false are **Non-Anomalous Account_IDs**.
