  ...
  if(plotName == "Plot of points distribution"){
    ...
    purrr::map (1:length(result),function(i){
      ...
      if(nrow(noOfTrueFalse)==2){
        ...
        anomaliesCountPlot[[i]] <- ggplot2::ggplot(noOfTrueFalse, aes(x=IsTrue, y=Frequency)) +
          geom_bar(stat = "identity")+
          labs(title="Distribution of points for the rule", 
              y = "Count")
    ...
    purrr::map (1:length(result),function(ruleNum){
      ...
      if(class(intermediateOutput)=="list"){
        ...
        plotAnomalies[[ruleNum]] <- ggplot2::ggplot(intermediateOutput, aes(x=Group, y=values)) +
          geom_bar(stat = "identity")+
          labs(title="Groups satisfying the rule", 
