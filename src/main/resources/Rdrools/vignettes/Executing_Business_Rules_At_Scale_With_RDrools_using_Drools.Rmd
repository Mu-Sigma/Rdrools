  ...
  if(plotName == "Plot of points distribution"){
    ...
    purrr::map (1:length(result),function(i){
      ...
      if(nrow(noOfTrueFalse)==2){
        ...
        anomaliesCountPlot[[i]] <- plotly::plot_ly(noOfTrueFalse, x=~IsTrue, y=~Frequency,
                                           type = "bar", width=800, height=500,
                                           marker = list(color = 'rgba(158,202,225,1)', 
                                           line = list(color = 'rgba(8,48,107, 0)'))) %>%
                                          plotly::layout(title = paste0("Distribution of points for the rule "),
                                                 xaxis = list(title = ""),
    ...
    purrr::map (1:length(result),function(ruleNum){
      ...
      if(class(intermediateOutput)=="list"){
        ...
        plotAnomalies[[ruleNum]] <- plotly::plot_ly(intermediateOutput, x=~Group, y=~values, 
                                                    type="bar", width=800, height=500, 
                                                    marker = list(color = 'rgba(158,202,225, 1)',
                                                    line = list(color = 'rgba(8,48,107, 0)', width = 2))) %>%
                                    plotly::layout(title = paste0("Groups satisfying the rule "),width = 900,
